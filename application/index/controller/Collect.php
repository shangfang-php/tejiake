...
class Collect extends UserCommon{
    public function index(){
        ...
        if($action == 'spread'){
            $condition['gc.is_spread'] = array('in','1,2');
        }else{
            $condition['gc.is_spread'] = 0;
        }
        $list = Db::name('goods_collect')
            ->alias('gc')
            ->field('gc.*,g.type,g.short_title,g.coupon_money,g.real_money,g.price,g.coupon_total_num,g.coupon_apply_num,g.end_time,g.taoke_money_percent,g.start_time')
            ->where($condition)
            ->join('goods g','gc.gid=g.id','left')
            ->order('gc.id desc')
            ->paginate(10,false,['query'=>request()->param()]);
        //exit;
        $collect_count = Db::name('goods_collect')->where(['is_spread'=>0])->count();
        $spread_count = Db::name('goods_collect')->where(['is_spread'=>array('in','1,2')])->count();
        $show = $list->render();
        $data = array(
            'action'       =>$action,
            'list'         =>$list,
